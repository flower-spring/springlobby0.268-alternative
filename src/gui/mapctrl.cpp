...
//		DrawStartRect(dc, -1, r, ColourDelta(col, -40), false, 180, false); /* box background */
		DrawStartRect(dc, -1, r, wxColour(255, 50, 50), false, 180, false); /* box background */
	...
	if (m_draw_start_types) {
		if (longval == IBattle::ST_Choose) {
			...
			if (m_tmp_brect.ally != -1) {
				...
				//DrawStartRect(dc, m_tmp_brect.ally, tmp, *wxYELLOW, false);
				DrawStartRect(dc, m_tmp_brect.ally, tmp, *wxBLACK, false);
			}
