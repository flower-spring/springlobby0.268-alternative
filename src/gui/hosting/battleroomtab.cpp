...
EVT_BUTTON(BROOM_RING_UNREADY_UNSYNC, BattleRoomTab::OnRingUnreadyUnsynced)
EVT_BUTTON(BROOM_STATUS, BattleRoomTab::OnStatus)
EVT_BUTTON(BROOM_PROMOTE, BattleRoomTab::OnPromote)
EVT_BUTTON(BROOM_BALANCE_MESSAGE, BattleRoomTab::OnBalanceMessage)
    ...
    , m_BattleActionSink(this, &UiEvents::GetUiEventSender(UiEvents::OnBattleActionEvent))
{
	...
	m_status_btn = new wxButton(this, BROOM_STATUS, _("!status"), wxDefaultPosition, wxSize(-1, CONTROL_HEIGHT));
	m_status_btn->SetToolTip(_("Send ''!status'' in the tchat"));
	...
	m_balance_btn = new wxButton(this, BROOM_BALANCE_MESSAGE, _("!balance"), wxDefaultPosition, wxSize(-1, CONTROL_HEIGHT));
	m_balance_btn->SetToolTip(_("Balance teams"));

	m_ring_btn = new wxButton(this, BROOM_RING_UNREADY_UNSYNC, _("!ring"), wxDefaultPosition, wxSize(-1, CONTROL_HEIGHT));
	m_ring_btn->SetToolTip(_("Ring the battle in this battleroom"));


	m_fixcolours_btn = new wxButton(this, BROOM_FIXCOLOURS, _("!fixcolors"), wxDefaultPosition, wxSize(-1, CONTROL_HEIGHT));
	m_fixcolours_btn->SetToolTip(_("Fixcolours"));
	...
	wxMenuItem* m_balance_message_mnu = new wxMenuItem(m_manage_users_mnu, BROOM_BALANCE_MESSAGE, _("Balance message"), _("Make auto balance"));
	m_manage_users_mnu->Append(m_balance_message_mnu);
	...
	m_preset_btns_sizer = new wxWrapSizer(wxHORIZONTAL);
	...
	m_buttons_sizer = new wxWrapSizer(wxHORIZONTAL);
	...
	m_buttons_sizer->Add(m_status_btn, 0, wxEXPAND | wxALL, 2);
	m_buttons_sizer->Add(m_promote_btn, 0, wxEXPAND | wxALL, 2);
	m_buttons_sizer->Add(m_balance_btn, 0, wxEXPAND | wxALL, 2);
	m_buttons_sizer->Add(m_fixcolours_btn, 0, wxEXPAND | wxALL, 2);
	m_buttons_sizer->Add(m_ring_btn, 0, wxEXPAND | wxALL, 2);
...
void BattleRoomTab::UpdateUser(User& user, bool userJustAdded)
{
	...
	if (bs.spectator) {
		...
		m_ally_sel->Disable();
		m_team_sel->Disable();
		m_autolaunch_chk->SetValue(false);
...
void BattleRoomTab::OnStatus(wxCommandEvent& /*unused*/)
{
	if (!m_battle)
		return;
	...
	if (m_battle->IsFounderMe()) {
		...
		m_battle->m_autohost_manager->GetAutohostHandler().Status();
	}
}
...
void BattleRoomTab::SetBattle(IBattle* battle)
{
	...
	m_status_btn->Enable(isBattleEnabled);
	m_balance_btn->Enable(isBattleEnabled);
	m_fixcolours_btn->Enable(isBattleEnabled);
	m_ring_btn->Enable(isBattleEnabled);
...
void BattleRoomTab::OnBalanceMessage(wxCommandEvent& /*unused*/)
{
	if (!m_battle)
		return;
	wxLogMessage(wxEmptyString);
